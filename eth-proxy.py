...
def on_shutdown(f):
    ...
    if os.path.isfile('aqua-proxy.pid'):
        os.remove('aqua-proxy.pid')
...
def on_connect(f):
    ...
    initial_job = (yield f.rpc('aqua_submitLogin', [settings.WALLET, settings.CUSTOM_EMAIL], 'Proxy_'+version.VERSION+debug))
...
def main():
    ...
    log.warning("Aquachain Stratum proxy version: %s" % version.VERSION)
